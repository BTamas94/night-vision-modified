...
class Cursor {
  ...
  xSync(hub, layout, props, update2) {
    ...
    console.log("LOCKED: ", layout.main);
  ...
  xValues(hub, layout, props) {
    if (!this.locked || this.meta.scrollLock) {
      ...
      this.vi = layout.main.y2vi(this.y);
    }
    let values = [];
    let vi;
...
class Input {
  ...
  async listeners() {
    const Hamster = await import("./hamster-dacf6a4a.js").then((n) => n.h);
    const Hammer = await import("./hammer-beeb5384.js").then((n) => n.h);
